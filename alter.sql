...
-- user2 er age column name change hoia user_age hobe.
ALTER TABLE user2
...
ALTER TABLE user2
    ADD constraint unique_user2_user_name UNIQUE(user_name);   -- ai command er maddhome user2 table a user_name column a unique constraint add kortesi. Aikhane unique_user2_user_name --> aitar dara bujhai: ki constraint add korbo, tableName, kon column a add korbo. Ai name gulo underscore dia add korte hobe. Tobe vul hose somossa nai. Aita bujhar jonno aivabe likhte hoi.

ALTER TABLE user2
    ADD constraint pk_user2_user_age PRIMARY KEY(user_id);  -- Amra chaila aikhanew primary key add korte pari.


ALTER TABLE user2
    DROP constraint unique_user2_user_name;   -- kono constraint k drop korte pari, aikhane unique_user2_user_name holo constraint create kora somoi jei name diasi, oi name. Ar manualy jodi ai constraint add na kori, tahole  edit table theke Index Name a automatic generate hioa thake, index name gulo.
